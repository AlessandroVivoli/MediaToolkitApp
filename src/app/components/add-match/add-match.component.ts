...
export class AddMatchComponent implements OnInit {
    ...
    this.form = new FormGroup(
      {
        ...
        player1Selected: new FormControl<PlayerModel>(null, [
          Validators.required,
        ]),
        player2Selected: new FormControl<PlayerModel>(null, [
          Validators.required,
        ]),
      },
      [this.validateMatch]
            ...
            [
              {
                ...
                points: this.player1Points.map((control) => control.value),
              },
              {
                id: this.player2.id,
                name: this.player2.name,
                points: this.player2Points.map((control) => control.value),
              },
            ],
            this.player1.name
          )
  ...
  get player1() {
    return (this.form.controls['player1Selected'] as FormControl<PlayerModel>)
      .value;
  }
  get player2() {
    return (this.form.controls['player2Selected'] as FormControl<PlayerModel>)
      .value;
  }
  ...
  private validateMatch(control: FormGroup): ValidationErrors {
    ...
    const player1Mapped = player1.controls.map((control, index) => {
      return (
        ...
        (control.value >= 10 &&
          player2.controls[index].value === control.value - 2)
      );
    });
    const player2Mapped = player2.controls.map((control, index) => {
      return (
        ...
        (control.value >= 10 &&
          player1.controls[index].value === control.value - 2)
      );
    });

    const validSets =
      player1Mapped.every((value, index) => value !== player2Mapped[index]) &&
      player2Mapped.every((value, index) => value !== player1Mapped[index]);
    ...
    const validWinner =
      (player1Win && player1Mapped[player1Mapped.length - 1]) ||
      (player2Win && player2Mapped[player2Mapped.length - 1]);

    return (player1Win || player2Win) && validSets && validWinner
      ? null
      : { invalidMatch: true };
  }
}
